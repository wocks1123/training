...
import torch
from models.mobilenet_ssd.mobilenet_ssd_priors import *
from models.mobilenet_ssd.MobileNetV2 import MobileNetV2_pretrained
...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes, backbone_net):
        ...
        if backbone_net == 'MobileNetV2' or 'mobilenetv2_q':
...
class AuxillaryConvolutions(nn.Module):
    ...
    def __init__(self, backbone_net):
        ...
        if backbone_net == "MobileNetV2":
          ...
          self.init_conv2d()
        elif backbone_net == "mobilenetv2_q":
          self.extras = ModuleList([
          Sequential(
              Conv2d(in_channels=1280, out_channels=256, kernel_size=1),
              ReLU(),
...
class SSD(nn.Module):
    def __init__(self, num_classes, backbone_network):
        ...
        #self.base_net = MobileNetV1().models
        ...
        if self.backbone_net == 'MobileNetV1':
            ...
            self.base_net = MobileNetV2_pretrained('data/weight/mobilenet_v2.pth.tar').model
        elif self.backbone_net == 'mobilenetv2_q':
            # for quantization
            self.base_net = MobileNetV2()
            state_dict = torch.load("data/weight/torchvision/mobilenet_v2.pth")
    ...
    def forward(self, image):
        ...
        # res = torch.cat([locs.squeeze(), class_scores.squeeze()], dim=1)
        # print("res", res.shape)
        # print("res", res)
...
models = SSD(20)
loc, classes = models.forward(img)
