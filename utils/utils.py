...
def transform(image, boxes, labels, difficulties, split):
    ...
    """
    assert split in {'TRAIN', 'TEST', 'VOCTEST'}
...
def save_checkpoint(epoch, epochs_since_improvement, model, optimizer, loss, best_loss, is_best, dest_dir):
    ...
    if epoch % 10 == 0:
        filename = f"{dest_dir}/epoch{epoch}_checkpoint.pth.tar"
        torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        filename = f"{dest_dir}/BEST_checkpoint.pth.tar"
        torch.save(state, filename)
...
def detect_objects(model, priors_cxcy, predicted_locs, predicted_scores, min_score, max_overlap, top_k, n_classes, device):
    """
...
def create_cifar10_test500():
    CIFAR10_TEST_DIR = "/DB/CIFAR-10/test"
    classes = os.listdir(CIFAR10_TEST_DIR)
    DEST_DIR = "/DB/CIFAR-10/test500"
    for class_name in classes:
        class_dir = os.path.join(CIFAR10_TEST_DIR, class_name)
        dest_dir = f"{DEST_DIR}/{class_name}"
        if os.path.exists(dest_dir) is not True:
            os.mkdir(dest_dir)

        for i in range(50):
            img_file = "{:04d}.jpg".format(i)
            img_path = f"{class_dir}/{img_file}"

            dest_path = f"{DEST_DIR}/{class_name}/{img_file}"
            print(img_path, dest_path)
            shutil.copyfile(img_path, dest_path)


def create_cifar10_test500_json():
    CIFAR10_CLASSES = [
        'airplane',
        'automobile',
        'bird',
    ...
    DATA_DIR = "/DB/CIFAR-10/test500"
    classes = os.listdir(DATA_DIR)
    dest_file = "test500.json"
    img_infos = []
    for class_name in classes:
        ...
        for file_name in files:
            ...
            print(img_path)
            img_infos.append({"path": img_path, "label": CIFAR10_CLASSES.index(class_name)})
    import json
    with open(dest_file, 'w') as j:
